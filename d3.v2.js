(function () {
  ...
  d3.random = {
    normal: function (mu, theta) {
      ...
      return function () {
        ...
        return mu + theta * x * Math.sqrt(-2 * Math.log(r) / r)
      }
    },
    logNormal: function (mu, theta) {
      var n = arguments.length
      if (n < 2) theta = 1
      if (n < 1) mu = 0
      var random = d3.random.normal()
      return function () {
        return Math.exp(mu + theta * random())
      }
    },
  ...
  d3.layout.force = function () {
    ...
    var force = {}, event = d3.dispatch('start', 'tick', 'end'), size = [ 1, 1 ], drag, alpha, friction = 0.9, linkDistance = d3_layout_forceLinkDistance, linkStrength = d3_layout_forceLinkStrength, charge = -30, gravity = 0.1, theta = 0.8, interval, nodes = [], links = [], distances, strengths, charges
    force.tick = function () {
      if ((alpha *= 0.99) < 0.005) {
        event.end({
          type: 'end',
  ...
  d3.layout.treemap = function () {
    ...
    var hierarchy = d3.layout.hierarchy(), round = Math.round, size = [ 1, 1 ], padding = null, pad = d3_layout_treemapPadNull, sticky = false, stickies, ratio = 0.5 * (1 + Math.sqrt(5))
  ...
  d3.geo.azimuthal = function () {
    ...
    azimuthal.invert = function (coordinates) {
      var x = (coordinates[0] - translate[0]) / scale, y = (coordinates[1] - translate[1]) / scale, p = Math.sqrt(x * x + y * y), c = mode === 'stereographic' ? 2 * Math.atan(p) : mode === 'gnomonic' ? Math.atan(p) : mode === 'equidistant' ? p : mode === 'equalarea' ? 2 * Math.asin(0.5 * p) : Math.asin(p), sc = Math.sin(c), cc = Math.cos(c)
  ...
  d3.geo.mercator = function () {
    function mercator (coordinates) {
      ...
      return [ scale * x + translate[0], scale * Math.max(-0.5, Math.min(0.5, y)) + translate[1] ]
    }
    var scale = 500, translate = [ 480, 250 ]
  ...
  d3.geo.circle = function () {
    ...
    var origin = [ 0, 0 ], degrees = 90 - 0.01, radians = degrees * d3_geo_radians, arc = d3.geo.greatArc().source(origin).target(d3_identity)
